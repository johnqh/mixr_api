...
// Enums
export const equipmentSubcategoryEnum = mixrSchema.enum(
  'equipment_subcategory',
  ['essential', 'glassware', 'garnish', 'advanced']
);

export const ingredientSubcategoryEnum = mixrSchema.enum(
  'ingredient_subcategory',
  ['spirit', 'wine', 'other_alcohol', 'fruit', 'spice', 'other']
);
...
export const recipeRatings = mixrSchema.table(
  'recipe_ratings',
  {
    id: serial('id').primaryKey(),
  ...
  table => ({
    uniqueUserRecipe: unique('unique_user_recipe').on(
      table.userId,
      table.recipeId
    ),
...
export const userFavorites = mixrSchema.table(
  'user_favorites',
  {
    userId: varchar('user_id', { length: 128 })
      .references(() => users.id, { onDelete: 'cascade' })
